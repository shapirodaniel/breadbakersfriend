...
function roundTo(myNum, numberOfPlaces) {
    ...
    if (myNum < 1 && numberOfPlaces == 0) { return '0'; }

    let numSpread = [...String(myNum)];
    let dotIndex = numSpread.indexOf('.');
    if (dotIndex == -1) { return String(myNum); }

    let afterDot_string = numSpread.slice(dotIndex + 1, );
    if (afterDot_string.length <= numberOfPlaces) { return String(myNum); }

    while (numSpread[0] == '0') { numSpread.shift(); }
    numSpread = numbify(numSpread.slice(0, dotIndex + numberOfPlaces + 2));
    if (numSpread[numSpread.length - 1] == 0) {
        numSpread.pop();
        if (numSpread[numSpread.length - 1] == '\.') { numSpread.pop(); }
    ...
    if (lastChar < 5) {
        numSpread.pop();
        while (numSpread[numSpread.length - 1] == 0) { numSpread.pop(); }
        if (numSpread[numSpread.length - 1] == '\.') { numSpread.pop(); }
        return numSpread.join('');
    }
    ...
    if (numSpread[numSpread.length - 1] == '\.') {
        ...
        if (numSpread.length == 1 && numSpread[0] < 10) { return numSpread.join(''); }
        ...
        numSpread[numSpread.length - 1] = numSpread[numSpread.length - 1] * (10 ** (startLen - finalLen));
        return numSpread.join('');
    }

    numSpread[numSpread.length - 1] += 1;
    while (numSpread[numSpread.length - 1] == 10 &&
...
function round_bySize(myNum) {
    ...
    let precision = (() => {
        switch (true) {
            case myNum >= 200:              return 0; 
            case myNum < 200 && myNum >= 5: return 1;
            case myNum < 5:                 return 2;
        }
    })();

    let res = roundTo(myNum, precision);
    if (res.match(/\.0+$|\.$/ig) !== null) {
...
function firstPass(myString, subrIndex = undefined, isItemWeight = false) {
    ...
    const numSplit_gram  = /\d(?=grams|gram|gr\.|gr|g\.|g(?=\s))/ig
    const numSplit_ounce = /\d(?=ounces|ounce|oz\.|oz(?=\s))/ig
    const numSplit_pound = /\d(?=pounds|pound|lbs\.|lbs|lb\.|lb|#)/ig
    let numSplit_list = [numSplit_gram, numSplit_ounce, numSplit_pound];

    const gramFinder  = /\sgrams|\sgram|\sgr\.|\sgr|\sg\.|\sg(?=\s)/ig;
    const ounceFinder = /\sounces|\sounce|\soz\.|\soz(?=\s)/ig;
    const poundFinder = /\spounds|\spound|\slbs\.|\slbs|\slb\.|\slb|\s#/ig;
    let finder_list = [gramFinder, ounceFinder, poundFinder];

    const multiSpaceFinder = /\s+/ig;

    function numSpacer (str, regexList) {
        for (let i=0; i<regexList.length; i++) {
            str = str.replace(regexList[i], match => {return match + ' ';});
        }
        return str;
    }
    ...
    function contextReplacer (str, regexList) {
        for (let i=0; i<regexList.length; i++) {
            str = str.replace(regexList[i], match => {
                switch (true) {
                    case match.match(gramFinder)  !== null: return ' g ';
                    case match.match(ounceFinder) !== null: return ' oz ';
                    case match.match(poundFinder) !== null: return ' # ';  
    ...
    s = contextReplacer(s, finder_list).replace(multiSpaceFinder, ' ').replace(/\,/ig, '');

    if (isItemWeight) { return s.split(' ').filter(val => { return val !== ''; }) }

    let warningDiv = subrIndex == undefined ?
        document.getElementById('main-recipe-warning') :
        document.getElementById(`subr-recipe-warning-${subrIndex}`);
    let numFlours = 0,
        numAsterisks = 0;
    let arr = s.split(' ').filter(val => { return val !== ''; });
    for (var i = 0; i < arr.length; i++) {
        if (arr[i].match(/flour/ig) !== null) { numFlours++; } else if (arr[i].match(/\*/ig) !== null) { numAsterisks++; }
    }
    let warning = (() => {
        switch (true) {
            case numAsterisks > 1:
...
function fracEval(myArray) {
    ...
    let fracRegex = /\d*\/\d*/g;
    return myArray.join(' ').replace(fracRegex, str => {
...
function simplifyCompoundImperial(myArray) {
    ...
    let grOzRegex = /^g$|^oz$/ig;
    let poundRegex = /^\#$/ig;
    let sum;

    for (var i = 0; i < myArray.length; i++) {
        try {
            if (typeof myArray[i] === "number" && myArray[i + 1].match(poundRegex) !== null &&
                typeof myArray[i + 2] === "number" && myArray[i + 3].match(grOzRegex) !== null) {
...
function parseUserInput(myString, subrIndex, isItemWeight = false) {
    ...
    return simplifyCompoundImperial(
        sumConsec(
            numbify(
                fracEval(
                    firstPass(myString, subrIndex, isItemWeight)
                ))));
}

function validateUserInput(myArray, subrIndex) {

    const unitRegex = /^g$|^oz$|^\#$/ig;
    let warningDiv = subrIndex == undefined ?
        document.getElementById('main-recipe-warning') :
        document.getElementById(`subr-recipe-warning-${subrIndex}`);
    ...
    if (typeof carr[0] !== 'number') {
        warningDiv.innerText = 'Oops! Please make sure your recipe begins with a number.';
        return;
    }
    ...
    if (str.match(/name_error/ig) !== null) {
        for (let i = 0; i < res.length; i++) {
            if (res[i][2].match(/name_error/ig) == null) { continue; }
    ...
    if (!isUniform) {
        for (let i = 0; i < res.length; i++) {
            if (res[i][1].match(/no_unit/ig) == null) { continue; }
...
function unitConvert(myArray) {
    ...
    for (var i = 0; i < myArray.length; i++) {
        ...
        } else if (myArray[i][1].match(/lbs|lb|\#/ig) !== null) {
...
function get_parsedInput(subrIndex = undefined) {
    ...
    let myString = subrIndex == undefined ?
        document.getElementById('main-recipe-textarea').innerText :
        document.getElementById(`subr-recipe-textarea-${subrIndex}`).innerText;
    let parsedInput = unitConvert(validateUserInput(parseUserInput(myString, subrIndex), subrIndex));
    return parsedInput;
}
...
function get_tmwArray(myArray) {
    ...
    const asteriskFinder = /\*/ig;
    const flourFinder = /flour/ig;
    let flourSum = 0,
        asterisk_loc, isAsterisk = false;

    for (var i = 0; i < myArray.length; i++) {
        if (myArray[i][2].match(flourFinder) !== null) { flourSum += myArray[i][0]; }
...
function get_bpArray(tmwArray) {
    ...
    let temp = [],
        res = [];
    for (let i = 0; i < tmwArray.length; i++) { temp.push(tmwArray[i][0] / tmwArray[0][0] * 100); }
...
function restore_originalInputString(subrIndex = undefined) {
    ...
    let textarea_selector = subrIndex == undefined ? '#main-recipe-textarea' : `subr-recipe-textarea-${subrIndex}`;
    if (original_inputString !== undefined) {document.querySelector(textarea_selector).innerText = original_inputString;}
}
...
function get_ingNameAndBPArray(subrIndex = undefined) {
    ...
    let mainOrSubr_warningDiv = subrIndex == undefined ? 'main-recipe-warning' : `subr-recipe-warning-${subrIndex}`;
    ...
    for (var i = 0; i < bp_arr.length; i++) {
        ...
        let bpVal = tmw_arr[i][2].match(/flour/ig) == null ? round_bySize(bp_arr[i]) : roundTo(bp_arr[i], 2);
    ...
    function adjust_flourSum(ingNameAndBPArray) {
        ...
        let is_asteriskNode = clone(ingNameAndBPArray).map(subarr => subarr[0]).some(val => val.match(/\*/ig) !== null);
        if (is_asteriskNode) { return ingNameAndBPArray; }

        /* match only 'flour' not preceded by 'total ' */
        let flourRows = ingNameAndBPArray.filter(row => { 
            return row[0].match(/flour/ig) !== null && row[0].match(/total/ig) == null; });
        if (flourRows.length == 0) { return ingNameAndBPArray; }
        ...
        let flourSum = Number(roundTo(flourValArray.reduce((a, b) => a + b, 0), 2));
        if (flourSum == 100) {
        ...
        console.log(`${bp_diff}% added to ${ingNameAndBPArray[i][0]}'s baker's percentage!`);
        return ingNameAndBPArray;
    }
...
function get_totalFormulaVal(ingNameAndBPArray) {
    ...
    let bpValArray = (() => {
        let res = [];
        ingNameAndBPArray.forEach(s_arr => { if (s_arr[0] !== 'total flour') { res.push(Number(s_arr[1])); } });
        return res;
    })();
