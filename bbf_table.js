...
function buildTable (ingNameAndBPArray, subrIndex = undefined) {
    ...
    if (ingNameAndBPArray == undefined) {
        console.log('looks like there\'s an issue with this recipe! please resubmit.');
        return;
    }

    let ingredients_body = subrIndex == undefined ? 
        document.getElementById('main-ingredients-body') :
        document.getElementById(`subr-ingredients-body-${subrIndex}`);
    let ingredientsRow_template = subrIndex == undefined ? 
    ...
    for (var i=0; i<numRows; i++) {
        ...
        newRow.id = subrIndex == undefined ? 'ingredients-row-' + i : 'subr-ingredients-row-' + i;
        ...
        newRow_cells.forEach(cell => {
            ...
            if (i == 0 && ingNameAndBPArray[0][0].search('total flour') !== -1) {cell.classList.add('tmw');}   
            else if (i == 0 && ingNameAndBPArray[0][0].match(/\*/ig) !== null)  {cell.classList.add('asterisk');}   
        });
        ...
        if (subrIndex == undefined) {
            ...
            newSubr.querySelectorAll('*').forEach(node => {
                if (node.id.endsWith('-')) {node.id += i;}
            });
            newSubr.querySelector('.recipe-name').innerText = Array.from(newSubr.previousElementSibling.children)[1].innerText;
        }
    }

    let tdw_val = get_queryTarget('tdw-val', subrIndex)[0];
    let tf_val = get_queryTarget('tf-val', subrIndex)[0];
    tdw_val.innerText = subrIndex == undefined ? 
        0 : Number(Array.from(document.getElementById(`subr-wrapper-${subrIndex}`).previousElementSibling.children)[3].innerText);
...
function cacheCurrentTable() {
    ...
    function getBodyContents(queryTarget, bodyClass) {
        ...
        let res = [], isSubr = queryTarget !== document && bodyClass == '.ingredients-body';
        let bodyChildren_array = Array.from(queryTarget.querySelector(bodyClass).children);
        if (!isSubr) {bodyChildren_array = bodyChildren_array.filter(child => {return !child.matches('.subr');})}
        bodyChildren_array.forEach(row => {
            row = Array.from(row.children);
            if (!isSubr) {row = row.slice(1,);}
    ...
    let subrecipeArray = [];
    let subr_wrapperArray = Array.from(document.querySelectorAll('.subr.table-container:not(.template)'))
        .filter(subr => {return subr.querySelector('.subr.ingredients-body').childElementCount > 0;});
...
function saveRecipe(fileType = 'json') {
    ...
    let currentTable = (() => {
        switch(fileType) {
            case 'spreadsheet': return spreadsheetify();
            case 'csv': return spreadsheetify(true);
            case 'json': return JSON.stringify(cacheCurrentTable());
        }
    })(); 
    let recipename_txt = document.querySelector('.recipe-name').innerText;
    downloadToFile(currentTable, `bbf_${recipename_txt}_${fileType}.txt`, 'text/plain');
}
...
function readJSONIntoTable(myRecipeObject) {
    ...
    for (let i = 0; i < ingredients.length; i++) {
        if (ingredients[i][0] === 'total flour') {continue;} 
    ...
    for (let i=0; i<items.length; i++) {
        ...
        let itemsRow_nodeArray = Array.from(document.querySelector('.items-body').lastElementChild.children).slice(1,);
        for (let j=0; j<itemsRow_nodeArray.length; j++) {itemsRow_nodeArray[j].innerText = items[i][j];}
    }
    
    /* add subrecipes */
    for (let i = 0; i < subrecipes.length; i++) {
        let main_ingName = subrecipes[i].main_ingName; 
        let main_ingName_node = Array.from(document.querySelectorAll('.ing-name:not(.header):not(.template):not(.subr)'))
            .filter(node => {return node.innerText == main_ingName;})[0];
        ...
        subrecipes[i].subr_ingNameAndBPArray.forEach(subarr => {
            ...
            if (!isTMW) {subr_inputString += subarr.slice(0,2).reverse().join(' ') + ' ';}
        });
        subr_wrapper.querySelector(`#subr-recipe-textarea-${subrIndex}`).innerText = subr_inputString;
        subr_wrapper.querySelector(`#subr-parse-recipe-button-${subrIndex}`).click();
    }
...
function loadRecipe_button() {document.getElementById('load-recipe-file-reader').click();}
...
function getSubrVal (mySubrecipeObject, key) {
    ...
    if (mySubrecipeObject == undefined) {return;}
    ...
    switch (key) {
        case 'recipe-name':     return subr_recipeName;
        case 'tdw-text':        return subr_tdw_text;
        case 'tdw-val':         return subr_tdw_val;
        case 'tf-text':         return subr_tf_text;
        case 'tf-val':          return subr_tf_val;
        case 'ingredients':     return subr_ingredients;
    }
}
...
function buildSS_String(isCSV = false, isSubr = false, subr_objIndex = undefined, final_rowCount = undefined) {
    ...
    let recipeName = isSubr == false ? currentTable.recipeName : getSubrVal(currentTable, 'recipe-name');
    let totalDoughWeight = isSubr == false? currentTable.totalDoughWeight : getSubrVal(currentTable, 'tdw-val');
    let totalFormula = isSubr == false? currentTable.totalFormula : getSubrVal(currentTable, 'tf-val');
    let ingredients = isSubr == false ? currentTable.ingredients : getSubrVal(currentTable, 'ingredients');
    ...
    /* header rows */
    let itemHeader = isCSV == true ? 'item name,weight,amount\n' : 'item name,weight (g),amount\n';
    let ingredientHeader = 'ingredient,baker\'s %,weight (g)\n';

    /* tdw, tf text */
    let tdw_text = isSubr == false ? 'total dough weight (g):' : getSubrVal(currentTable, 'tdw-text');
    let tf_text = isSubr == false ? 'total formula %:' : getSubrVal(currentTable, 'tf-text');

    /* initialize string, blankRow, rowCount -- !important:
    (used to determine formulas for SW col) */
    let ss_CSV = new String();
    let blankRow = ',,,\n';
    final_rowCount == undefined ? final_rowCount = 1 : final_rowCount = final_rowCount;
    ...
    if (isSubr == false) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            let itemWeight = isCSV == true ? 
                items[i][1] : 
                unitConvert([parseUserInput(items[i][1] + ' ' + itemName, undefined, true)]).flat()[0]
    ...
    totalDoughWeight = isCSV == true ? totalDoughWeight : (() => {
        let res;
        if (items == undefined || items.length == 0) {
            /* C50 arbitrarily ensures coverage for main table ingredient column match function */ 
            res = isSubr == false ? 
                0 : `"=index(A1:C50, match(""${recipeName}"", A1:A50, 0), 3)"`;
        } else {
            res = isSubr == false ? 
                '"=sumproduct(\$B' + itemStartIndex + ':\$B' + itemEndIndex + ', \$C' + itemStartIndex + ':\$C' + itemEndIndex + ')"' : 
    ...
    let bpStartIndex = ingredients[0][0] === 'total flour' ? rowCount + 1 : rowCount;
    for (i = 0; i < ingredients.length; i++) {

        /* precision is the rounding parameter in spreadsheet =ROUND(INPUT, PRECISION) formula */
        let precision = (() => {
            switch (true) {
                case Number(ingredients[i][2]) >= 100:      return 0;
                case Number(ingredients[i][2]) < 100 
                     && Number(ingredients[i][2]) >= 10:    return 1;
                case Number(ingredients[i][2]) < 10     
                     && Number(ingredients[i][2]) >= 1:     return 2;
                case Number(ingredients[i][2]) < 1:         return 3;
            }
        })();
        ...
        let BP_formula = ingredients[i][0] === 'total flour' ? 'TOTAL-FLOUR-BP-FLAG' : ingredients[i][1];
        let SW_formula = ingredients[i][0] === 'total flour' ? 'TOTAL-FLOUR-SW-FLAG' : 
            '"=round(\$B' + rowCount + '/' + tf_location + '\*' + tdw_location + ',' + precision + ')"';
    ...
    totalFormula = isCSV == true ? totalFormula : `"=round(sum(B${bpStartIndex}:B${bpEndIndex}), 3)"`;

    /* find all matches of 'flour' rows and replaces TOTAL-FLOUR-SW-FLAG 
    with formula structured: ex., "=sumif(A9:A15, "*flour*", C9:C15)" */
    let totalFlour_BP_formula =
        `"=sumif(\$A${bpStartIndex}:\$A${bpEndIndex}, ""\*flour\*"", \$B${bpStartIndex}:\$B${bpEndIndex})"`; 
    let totalFlour_SW_formula = 
        `"=sumif(\$A${bpStartIndex}:\$A${bpEndIndex}, ""\*flour\*"", \$C${bpStartIndex}:\$C${bpEndIndex})"`;
...
function spreadsheetify(isCSV = false) {
    ...
    function getSS_String (n = 0) {
        ...
        if (n == 0) {
            ...
            console.log('final rowCount is: ', final_rowCount);
            return ss_String;
        }
    }
