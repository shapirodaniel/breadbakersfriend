...
const emptyCellRegex = /![a-z]*[A-Z]*[0-9]*|![\[\]\{\}\!\@\#\$\%\^\&\*\(\)\<\>\<\>\/\?\'\"\~\`]|^\s+$|^$/ig;
...
function saveState() {
    ...
    if (last_action !== undefined) {
        let justUndo = last_action.matches('#undo-toolbar-button, #undo-toolbar-icon, #undo-toolbar-text');
        let justRedo = last_action.matches('#redo-toolbar-button, #redo-toolbar-icon, #redo-toolbar-text');
        if (!(justUndo || justRedo)) { redoStack = []; }
    }
    ...
    if (undoStack[undoStack.length - 1].isEqualNode(undoStack[undoStack.length - 2])  && undoStack.length > 0 ) {
        undoStack.pop();
    }
}
...
function undoRedoFunctionality(e) {
    ...
    undoStack = undoStack.filter(state => {return state !== undefined;});
    redoStack = redoStack.filter(state => {return state !== undefined;});

    if (undoStack.length == 0 && is_undoClick) { return; }
    if (is_undoClick) {
    ...
    if (redoStack.length == 0 && is_redoClick) { return; }
    if (is_redoClick) {
...
function get_queryTarget(queryType, subrIndex = undefined) {
    ...
    let queryTarget = subrIndex == undefined ? document : document.querySelector(`#subr-wrapper-${subrIndex}`);
    let selector = subrIndex == undefined ? `:not(.subr).${queryType}` : `.subr.${queryType}`;
...
function get_nodeList(queryType, subrIndex = undefined) {
    ...
    return get_queryTarget(queryType, subrIndex)
        .filter(node => {
            return node.id.match(/header/ig) == null &&
                !node.classList.contains('template');
        });
}

/* returns true if any node has warning class applied, used by updaters */
function isWarning() { return Array.from(document.querySelectorAll('.warning')).length > 0 }
...
function apply_warningChanges(userInputCell, flourSum = 100) {
    ...
    let path_fromSubrButton = Array.from(document.querySelectorAll('.subr-parse-recipe-button:not(.template)'))
        .some(btn => { return btn.isEqualNode(userInputCell); });
    if (userInputCell == undefined || path_fromSubrButton) { userInputCell = get_queryTarget('tdw-val')[0]; }
    ...
    if (flourSum == 100) {
        CEnode_array.forEach(node => { node.classList.remove('warning'); });
        adjust_contentEditability();
        return;
    }
    let flourBP_nodeArray = CEnode_array.filter(node => {
        let is_flour_ingBP = node.matches('.ing-bp:not(.header):not(.template)') && node.previousElementSibling.innerText.match(/flour/ig) !== null;
        let isChild_of_targetParent = is_flour_ingBP && node.parentElement.parentElement.isEqualNode(userInputCell.parentElement.parentElement);
        return isChild_of_targetParent;
    });
    flourBP_nodeArray.forEach(node => { node.classList.add('warning'); });
    adjust_contentEditability();
}
...
function calculateTDW() {
    ...
    let is_validNum = !isNaN(Number(tdw_val.innerText)) && tdw_val.innerText.match(emptyCellRegex) == null;
    if (noItemRows && is_validNum) { return roundTo(Number(tdw_val.innerText), 0); }

    function isEmpty_innerText(node) { return node.innerText.match(emptyCellRegex) !== null; }

    function drop_firstVal(arr) { arr.forEach(subarr => { subarr.shift(); }) }

    let itemWeightArray = Array.from(document.querySelectorAll('.items-weight:not(.header):not(.template)'));
    ...
    let tdw = 0,
        itemName, converted_itemWeight, itemAmount;

    while (itemWeightArray.length > 0) {

        let emptyWeight_andOr_emptyAmount = (() => {
            return isEmpty_innerText(itemWeightArray[0]) || isEmpty_innerText(itemAmountArray[0]);
        })();
        ...
        itemName = itemNameArray[0].innerText.match(emptyCellRegex) !== null ?
            'item_name' : itemNameArray[0].innerText;
        converted_itemWeight = unitConvert(
            [parseUserInput(itemWeightArray[0].innerText + ' ' + itemName, undefined, true)]
        ).flat()[0];
        if (isNaN(converted_itemWeight)) { apply_warningChanges(itemWeightArray[0]) }

        itemAmount = Number(itemAmountArray[0].innerText);
        if (isNaN(itemAmount)) { apply_warningChanges(itemAmountArray[0]) }
...
function updateTDW(userInputCell) {
    ...
    let tdw_val = get_queryTarget('tdw-val')[0];
    if (userInputCell == undefined) { userInputCell = tdw_val }
...
function updateBP(userInputCell, subrIndex = undefined) {
    ...
    let flour_nodeList = bp_nodeList.filter(node => {
        let is_flourNode = node.previousElementSibling.innerText.match(/flour/ig) !== null;
        let not_totalFlour = !node.matches('.tmw');
        return is_flourNode && not_totalFlour;
    });
    let flourSum = flour_nodeList.map(node => Number(node.innerText)).reduce((a, b) => a + b, 0);
    let ingBody_selector = subrIndex == undefined ? 
        '.ingredients-body' : `#subr-ingredients-body-${subrIndex}`;
    let tmwBP_index = subrIndex == undefined ? 2 : 1;
    let tmw_ingBP = Array.from(document.querySelector(ingBody_selector).firstElementChild.children)[tmwBP_index];

    /* set 'total flour' bp to flourSum */
    if (tmw_ingBP.classList.contains('tmw')) { 
        tmw_ingBP.innerText = roundTo(flourSum, 3).replace(/\.$|\.0+$/ig, ''); 
    }

    /* important! if asterisk present, flourSum = 100 to avoid warning if flour ingredients present */
    if (tmw_ingBP.classList.contains('asterisk')) {flourSum = 100}

    apply_warningChanges(userInputCell, flourSum);
    if (isWarning()) { return; }

    let tf_val = get_queryTarget('tf-val', subrIndex)[0];
    tf_val.innerText = roundTo(bp_nodeList.filter(node => {
            return !node.classList.contains('tmw');
        })
        .map(node => Number(node.innerText)).reduce((a, b) => a + b, 0), 3);

    userInputCell.matches('.subr.ing-bp') ?
        (() => {
            let sw_nodeList = document.getElementById(`subr-wrapper-${subrIndex}`).querySelectorAll('.subr.ing-sw:not(.header):not(.template)');
...
function get_inputString(nodeType, subrIndex = undefined) {
    let temp = [];
    let nodeType_nodeList = get_nodeList(nodeType, subrIndex).filter(node => { return !node.classList.contains('tmw'); });
    let name_nodeList = get_nodeList('ing-name', subrIndex).filter(node => { return !node.classList.contains('tmw'); });
    while (nodeType_nodeList.length > 0) {
        temp.push([nodeType_nodeList[0].innerText, name_nodeList[0].innerText]);
        nodeType_nodeList.shift();
        name_nodeList.shift();
    }
    let textarea = subrIndex == undefined ?
...
function calculateSW(ingNameAndBPArray, sw_nodeList, subrIndex = undefined) {
    ...
    for (var i = 0; i < sw_nodeList.length; i++) {
        sw_nodeList[i].innerText =
            round_bySize(Number(ingNameAndBPArray[i][1]) / Number(tf_val.innerText) * Number(tdw_val.innerText));
    }
}

/* check if sw nodes are zeroed out, used by updateSW(), update_subrSW() */
function sumOf_swVals_fromTable(swNodes_array) {
    return swNodes_array.filter(node => {
        return !node.classList.contains('tmw');
    }).map(node => Number(node.innerText)).reduce((a, b) => a + b, 0);
}
...
function update_subrSW(userInputCell, subrIndex = undefined) {
    ...
    if (!userInputCell.matches('.subr')) {
        ...
        adjust_subrSW_difference(document.getElementById(`subr-wrapper-${subrIndex}`));
        return;
    }

    subr_tdw_val.innerText = nodeType == 'ing-sw' && subrIndex !== undefined ?
        roundTo(sumOf_swVals_fromTable(subr_sw_nodeList), 3) :
        Array.from(document.getElementById(`subr-wrapper-${subrIndex}`).previousElementSibling.children)[3].innerText;

    /* assign new bpVals to each bpNode, then call calculateSW() */
    for (let i = 0; i < subr_bp_nodeList.length; i++) { subr_bp_nodeList[i].innerText = subr_ingNameAndBPArray[i][1]; }
...
function updateSW(userInputCell, subrIndex = undefined) {
    ...
    let is_zeroedOut_asteriskSW = userInputCell == undefined ?
        false : userInputCell.classList.contains('asterisk') && Number(userInputCell.innerText) == 0;
    ...
    let sw_allZeroes_after_tdwChange =
        ...
        get_nodeList('ing-sw', subrIndex).map(node => Number(node.innerText)).reduce((a, b) => a + b, 0) == 0;
    if (userInputCell !== undefined && !sw_allZeroes_after_tdwChange) {
        let queryTarget = subrIndex == undefined ? 'main-ingredients-body' : `subr-ingredients-body-${subrIndex}`;
        let ingSW_selectorModifier = subrIndex == undefined ? ':not(.subr)' : '';
        let flour_nodes = Array.from(document.getElementById(queryTarget)
            .querySelectorAll(`.ing-sw:not(.header)${ingSW_selectorModifier}`)).filter(node => {
                let is_flourNode = node.previousElementSibling.previousElementSibling.innerText.match(/flour/ig) !== null;
                let not_totalFlour = !node.matches('.tmw');    
                return is_flourNode && not_totalFlour;
            })
        let is_zeroedOut_allFlours = roundTo(flour_nodes.map(node => Number(node.innerText)).reduce((a, b) => a + b, 0), 0) == 0;
    ...
    let new_mainIng_swVal, mainTable_sw_nodeList = get_nodeList('ing-sw');
    if (subrIndex !== undefined) {
        new_mainIng_swVal = userInputCell.matches('.ing-bp') ? null : update_subrSW(userInputCell, subrIndex);
        let mainIng_swNode = Array.from(document.getElementById(`subr-wrapper-${subrIndex}`).previousElementSibling.children)[3];
        mainIng_swNode.innerText = new_mainIng_swVal == null ? mainIng_swNode.innerText : new_mainIng_swVal;
    }
    ...
    let main_ingNameAndBPArray = (() => {
        let path_fromMainButton = (() => {
            return userInputCell == undefined ?
                false : userInputCell.isEqualNode(document.getElementById('main-parse-recipe-button'));
        })();
        let direct_tdwChange = (() => {
            return userInputCell == undefined ?
                false : userInputCell.isEqualNode(get_queryTarget('tdw-val')[0]);
        })();
        swSum = (path_fromMainButton || direct_tdwChange) ?
            Number(userInputCell.innerText) :
            sumOf_swVals_fromTable(mainTable_sw_nodeList);
        let path_fromUpdateBP = (() => {
            return userInputCell == undefined ?
                false : userInputCell.classList.contains('ing-bp');
        })();
        ...
        let is_allZeros_swColumn = sumOf_swVals_fromTable(mainTable_sw_nodeList) == 0;
        direct_tdwChange || is_allZeros_swColumn ? get_inputString('ing-bp') : get_inputString('ing-sw');
        return get_ingNameAndBPArray();
    })();
    ...
    (() => {
        ...
        let ingNameAndBP_cellArray = (() => {
            let res = [];
            let names = Array.from(ingredients_body.querySelectorAll('.ing-name:not(.header):not(.subr):not(.template)'));
            let bps = Array.from(ingredients_body.querySelectorAll('.ing-bp:not(.header):not(.subr):not(.template)'));
        ...
        get_queryTarget('tf-val')[0].innerText = get_totalFormulaVal(main_ingNameAndBPArray);
    })();
    ...
    let subr_wrapperArray = Array.from(document.querySelectorAll('.subr.wrapper:not(.template)')).filter(
        container => { return container.querySelector('.subr.ingredients-body').childElementCount > 0; });
    for (let i = 0; i < subr_wrapperArray.length; i++) {
        let subrIndex = get_subrIndex(subr_wrapperArray[i]);
        let subr_tdw_val = get_queryTarget('tdw-val', subrIndex)[0];
        let mainIng_swNode = Array.from(subr_wrapperArray[i].previousElementSibling.children)[3];
...
function adjust_subrSW_difference(subr_wrapper) {
    ...
    let mainIng_swVal = Array.from(subr_wrapper.previousElementSibling.children)[3];
    let subr_tdw_val = subr_wrapper.querySelector('.subr.tdw-val');
    let swVal_diff = Number(roundTo(Number(mainIng_swVal.innerText) - Number(subr_tdw_val.innerText), 2));

    if (swVal_diff == 0) { return; }

    let subrSW_nodes_not_tmw = Array.from(subr_wrapper.querySelectorAll('.subr.ing-sw:not(.header):not(.tmw):not(.asterisk):not(.template)'));
    let valArray = subrSW_nodes_not_tmw.map(node => Number(node.innerText));
    let largestVal = valArray[0],
        largestVal_index = 0;
    for (let i = 1; i < valArray.length; i++) {
        if (largestVal > valArray[i]) { continue; }
        largestVal = valArray[i];
        largestVal_index = i;
    }
    subrSW_nodes_not_tmw[largestVal_index].innerText = Number(subrSW_nodes_not_tmw[largestVal_index].innerText) + swVal_diff;
...
var CE_classNames = '.recipe-name, .method, .tdw-val, .items-name, .items-weight, .items-amount, .ing-bp, .ing-sw';

/* tests whether e.target is a contentEditable node */
function is_CEnode(userInputCell) {

    let CE_className_array = CE_classNames.replace(/\./ig, '').replace(/\,/ig, '').split(' ');
    return CE_className_array.some(name => { return userInputCell.classList.contains(name); });
}

/* sets userInputCell equal to zero if content is whitespace, empty string */
function writeZero_toEmptyCell(cell) {
    if (cell.innerText.match(emptyCellRegex) && !cell.matches('.recipe-name, .method-container, .items-name')) { cell.innerText = 0 }
}
...
function isButtonType(buttonID, e) {
    return e.target.matches(`#${buttonID}`) || document.getElementById(buttonID).contains(e.target);
}
...
function toggle_className(node, className) {
    ...
    let style1, style2;
    switch (className) {
        case 'not_editable': (() => { style1 = 'not_editable'; style2 = 'editable'; })(); break;
        case 'editable': (() => { style1 = 'editable'; style2 = 'not_editable'; })(); break;
        case 'visible': (() => { style1 = 'visible'; style2 = 'hidden'; })(); break;
        case 'hidden': (() => { style1 = 'hidden'; style2 = 'visible'; })(); break;
        case 'hasArrow': (() => { style1 = 'hasArrow'; style2 = 'noArrow'; })(); break;
        case 'noArrow': (() => { style1 = 'noArrow'; style2 = 'hasArrow'; })(); break;
        case 'active': (() => { style1 = 'active'; style2 = 'inactive'; })(); break;
        case 'inactive': (() => { style1 = 'inactive'; style2 = 'active'; })(); break;
    }
    let hasStyle = node.classList.replace(style2, style1);
    if (!hasStyle) { node.classList.add(style1); }
}
...
function apply_CEstyle() {
    ...
    if (isWarning()) {
        let CEnodes_noWarning = Array.from(document.querySelectorAll(CE_classNames)).filter(node => {
    ...
    Array.from(document.querySelectorAll(CE_classNames))
        .filter(node => { return !node.matches('.template, .header, .tmw, .ing-bp.asterisk, .subr.tdw-val, .subr.recipe-name') })
        .forEach(node => {
    ...
    if (get_queryTarget('tdw-val')[0].innerText == 0) {
        Array.from(document.querySelectorAll('.ing-sw:not(.header):not(.tmw):not(.template)')).forEach(node => {
            node.contentEditable = 'false';
            toggle_className(node, 'not_editable');
        });
    }
    ...
    if (document.getElementById('items-body').childElementCount > 0) {
        Array.from(document.querySelectorAll('.tdw-val, .ing-sw:not(.header):not(.tmw):not(.template)')).forEach(node => {
            node.contentEditable = 'false';
            toggle_className(node, 'not_editable');
        })
    }
...
function validateEntry(e) {
    ...
    if (e.target.matches('.recipe.container') || e.target.matches('.method.container')) { return; }
    if (e.target.innerText == initial_cellValue) { return; }

    let subr_wrapperArray = Array.from(document.querySelectorAll('.subr.wrapper:not(.template)'));
    let possible_subrMatch = subr_wrapperArray.filter(node => { return node.contains(e.target); })[0];
    let subrIndex;
    if (possible_subrMatch !== undefined) { subrIndex = get_subrIndex(possible_subrMatch); }

    writeZero_toEmptyCell(e.target);
    e.target.innerText = e.target.innerText.trim();

    let is_main_tdwCell = e.target.isEqualNode(document.getElementById('tdw-val'));
    let is_itemWeightOrAmountCell = e.target.matches('.items-weight') || e.target.matches('.items-amount');
    ...
    switch (true) {
        ...
        case is_main_tdwCell: updateTDW(e.target); break;
        case is_itemWeightOrAmountCell: updateTDW_fromItems(); break;
        case is_bpCell: updateBP(e.target, subrIndex); break;
        case is_swCell: updateSW(e.target, subrIndex); break;
    }
}

/* set initial_cellValue if is_CEnode */
function focusinHandler(e) { if (is_CEnode(e.target)) { initial_cellValue = e.target.innerText; } }
...
function clickHandler(e) {
    ...
    if (isButtonType('undo-toolbar-button', e) || isButtonType('redo-toolbar-button', e)) {
        last_action = isButtonType('undo-toolbar-button', e) ?
            document.querySelector('#undo-toolbar-button') : document.querySelector('#redo-toolbar-button');
    }
    ...
    if (!is_saveMenuButton && !isButtonType('save-toolbar-button', e)) { toggle_className(save_dropMenu, 'hidden'); }

    switch (true) {
        
        case is_CEnode(e.target): break;
        
        case e.target.matches('#main-parse-recipe-button'): mainRecipe_button(); undoStack = []; break;
        case e.target.matches('#main-load-recipe-button'): loadRecipe_button(); undoStack = []; break;
        case e.target.matches('.subr-parse-recipe-button'): subRecipe_button(e); break;
        case e.target.matches('.remove-subrecipe-button'): remove_subRecipe(e.target); break;
        case e.target.matches('.remove-items-button'): remove_itemRow(e.target); break;
        
        case isButtonType('undo-toolbar-button', e) || isButtonType('redo-toolbar-button', e): undoRedoFunctionality(e); break;
        case isButtonType('add-items-button', e): add_itemRow(); break;
        case isButtonType('load-toolbar-button', e): loadRecipe_button(); undoStack = []; break;
        case isButtonType('save-toolbar-button', e): save_dropMenu.matches('.hidden') ? toggle_className(save_dropMenu, 'visible') : toggle_className(save_dropMenu, 'hidden'); break;
        ...
        case save_dropMenu.contains(e.target):
            switch (true) {
                case isButtonType('ios-user-save-message', e): 
                    ...
                    break;
                case isButtonType('save-spreadsheet', e): saveRecipe('spreadsheet'); break;
                case isButtonType('save-csv', e): saveRecipe('csv'); break;
                case isButtonType('save-json', e): saveRecipe(); break;
            }
            break;
        case e.target.matches('#ios-user-modal-close-button'): document.querySelector('#ios-user-modal').classList.toggle('active'); break;
        
        case e.target.matches('.arrowbox:not(.subr):not(.header):not(.template):not(.tmw):not(.asterisk)') ||
             e.target.matches('.ing-name:not(.subr):not(.header):not(.template):not(.tmw):not(.asterisk)'):
        ...
        case e.target.matches('.quickstart-link span'): document.querySelector('#quickstart-modal').classList.toggle('active'); break;
        case e.target.matches('#quickstart-modal-close-button'): document.querySelector('#quickstart-modal').classList.toggle('active'); break;
        
        case e.target.matches('.topic-button') || e.target.matches('.topic-button-text') || e.target.matches('.topic-button-caret'): 
        ...
        case e.target.matches('.faq-link'): 
            ...
            break;
        case e.target.matches('#faq-modal-close-button'): document.querySelector('#faq-modal').classList.toggle('active'); break;
    }

    if (!e.target.matches('#main-parse-recipe-button')) { saveState(); }

    /* store last visited node for undoRedo functionality */
    if (!is_CEnode(e.target)) { last_action = e.target; }
}
    ...
    switch (true) {
        ...
        case e.keyCode == 13 && isRecipeTextarea:
            ...
            e.target.parentElement.matches('.subr') ?
                e.target.parentElement.querySelector('.subr-parse-recipe-button').click() :
                e.target.parentElement.querySelector('.main-parse-recipe-button').click();
            break;
...
function assignEventListeners() {
    ...
    console.log('event listeners have been assigned!');
}
...
function get_parentNode_forRemoval(cell) {
    let queryTarget = cell.matches('.remove-subrecipe-button') ? '.subr.wrapper:not(.template)' : '.items.row:not(.template)';
    return Array.from(document.querySelectorAll(queryTarget)).filter(wrapper => { return wrapper.contains(cell) })[0];
}
...
function mainRecipe_button() {
    ...
    let err;
    try { buildTable(get_ingNameAndBPArray()); } catch (err) {
        if (document.getElementById('main-recipe-textarea').innerText == '') {
            console.log('Oops! Recipe submit field is empty, please resubmit.');
            return;
        }
        console.log('Oops! Parse failed, exiting...');
        return;
    }
...
function toggle_mainIng_arrow(subr_wrapper) {
    ...
    let arrowContainer = subr_wrapper.previousElementSibling.querySelector('.arrowbox');
    let hasRows = subr_wrapper.querySelector('.subr.ingredients-body').childElementCount > 0;
...
function subRecipe_button(e) {
    ...
    let subr_userInputContainer = subr_wrapper.querySelector('.subr.user-input-container');
    let subr_tableContainer = subr_wrapper.querySelector('.subr.table-container');

    let err;
    try { buildTable(get_ingNameAndBPArray(subrIndex), subrIndex); } catch (err) {
        if (document.getElementById(`subr-recipe-textarea-${subrIndex}`).innerText == '') {
            console.log('Oops! Recipe submit field is empty, please resubmit.');
            return;
        }
        console.log('Oops! Parse failed, exiting...');
...
function add_itemRow() {
    ...
    let new_itemRow = document.getElementById('items-row-template').cloneNode(true);
    let new_itemRow_childArray = Array.from(new_itemRow.children);

    if (numItems == 0) { toggle_className(itemsContainer, 'visible'); }
    ...
    new_itemRow_childArray.forEach(child => { child.classList.remove('template'); });
...
function remove_itemRow(removeButton) {
    ...
    if (itemsBody.childElementCount == 0) {
        ...
        Array.from(document.querySelectorAll('.ing-sw:not(.header):not(.template)'))
            .forEach(node => { node.innerText = 0; });
        apply_CEstyle();
        return;
    }
